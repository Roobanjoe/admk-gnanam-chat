...
void main() {
  ...
  float dynamicAmplitude = uAmplitude + (mouseEffect * uMouseInfluence);
  
  float height = snoise(vec2(uv.x * 2.0 + uTime * 0.1, uTime * 0.25)) * 0.5 * dynamicAmplitude;
  height = exp(height);
  ...
  vec3 auroraColor = intensity * rampColor * (1.0 + mouseEffect * 0.5);
...
export default function Aurora(props: AuroraProps) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      setMousePos({
        ...
        y: 1.0 - (e.clientY / window.innerHeight) // Flip Y coordinate
      });
    };
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uTime.value = t * 0.001 * (currentProps.speed ?? speed);
  ...
  }, [amplitude, mousePos.x, mousePos.y, colorStops, blend, speed, mouseInfluence]);
